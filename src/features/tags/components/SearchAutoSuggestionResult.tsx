...
function SearchAutoSuggestionResult({
  isCollapsed,
  tags,
}: SearchAutoSuggestionResultProps) {
  return (
    <div
      className={`flex flex-col border-2 dark:border-none dark:bg-gray-600 px-4 py-2.5 transition-transform origin-top transform-[scaleY(0%)] ${!isCollapsed && 'transform-[scaleY(100%)]'}`}
    >
      <div className={`flex flex-col gap-2`}>
        ...
        <ul className="flex gap-2 flex-wrap">
          {tags.map((tag) => (
            <PostTag key={tag.label} label={tag.label} />
          ))}
        </ul>
      </div>
    </div>
