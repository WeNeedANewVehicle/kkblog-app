...
function useDeleteCommentModal() {
  ...
  const onOpenDeleteCommentModal = useCallback(({ id, postId, parentCommentId }: DeleteCommentDto) => {
    open({
        isOpen: true,
        onClose: close,
        onConfirm: () => deleteComment({ id, postId, parentCommentId }, { onSuccess: () => close() }),
        title: '알림',
