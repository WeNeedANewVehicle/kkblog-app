...
function useLoadTempPosts() {
    ...
    const [selectedTempPost, setSelectedTempPost] = useState<GetMyPostsItemResponseDto>();
    ...
    const onOpenTempPostModal = useCallback((e: BaseSyntheticEvent) => {
        e.preventDefault();
        openTempPostModal({ ...props, selectedTempPost });

    }, [openTempPostModal, closeTempPostModal, onChangeRadio, tempPosts, selectedTempPost, props])
