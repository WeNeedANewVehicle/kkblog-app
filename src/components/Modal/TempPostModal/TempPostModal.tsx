...
export interface TempPostModalProps
...
function TempPostModal({
  ...
  const onConfirm = useCallback(() => {
    if (selectedTempPost) {
      openConfirm(
        '지금까지 작성한 내용을 모두 잃게됩니다. 정말 임시글을 불러오시겠습니까?',
        () => {
          onClose()
  ...
  return (
    ...
    >
      <p>아직 작성되지 않은 글 목록을 불러옵니다.</p>
      <ul className="flex flex-col gap-2 overflow-y-scroll">
        {tempPosts.map((post) => (
          <TempPostModalItem key={post.id} {...post} onChange={onChangeRadio} />
        ))}
      </ul>
    </ConfirmModal>
  )
