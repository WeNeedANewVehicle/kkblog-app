...
function SeoModal({ onClose, isOpen, ...rest }: SeoModalProps) {
  return (
    <Modal className={`flex flex-column flex-1 gap-1 ${style.wrapper} ${!isOpen && style.closed}`} isOpen {...rest}>
      ...
      <form className="flex flex-column gap-1">
        <div>
          <LabeledText label="썸네일 이미지">
            <div className={`flex justify-center items-center ${style.preview}`}>
              <label className={`flex sm ${buttonCss.black}`} htmlFor='thumbnail'>
                등록하기
              </label>
            </div>
